...
# 持久化存储 last_update_id 路径
last_update_file="/root/.last_update_id"

# 如果文件不存在，初始化 last_update_id
if [ ! -f "$last_update_file" ]; then
    echo "Initializing last_update_id to 0" >> /root/bot_listener_debug.log
...
# 获取 Telegram 更新并循环处理
while true; do
    # 获取未处理的消息，使用 last_update_id 作为 offset，确保我们从上次处理的地方开始
    ...
    # 检查返回的数据是否为空
    if [ "$(echo $RESPONSE | jq -r '.result | length')" == "0" ]; then
    ...
    # 每次检查后等待 2 秒，避免过度调用 Telegram API
    sleep 2
done
